%{
    ...
    vector< stack_entry > current_stack;
...
TypeName:
    Identifier  {   
                    find_variable_in_class(($1).token, temp_stack);
                    ($$).token = ($1).token;
                }
|   TypeName Dot Identifier { 
                                ($$).token = ($1).token + "." + ($3).token;
                            }
;
...
VariableDeclarator:
    VariableDeclaratorId Assign VariableInitializer {    
                                                        ...
                                                        if(!check_return_type(($1).type),($3).type){
                                                            cerr<<"Error: Cannot assign "<<($3).type<<" to "<<($1).type<<endl;
                                                            exit(1);
                                                        }
                                                    }
...
FormalParameterList:
    FormalParameterList Comma FormalParameter {  ($$).argument_type = ($1).argument_type + "," + ($3).argument_type; }
|   FormalParameter { ($$).argument_type = ($1).argument_type;}
;
...
ConstructorDeclaration:
    Modifiers Declarator { add_constructor(($2).token, ($2).argument_type, "", ($1).modifier, ($2).scope); } ConstructorBody { }
|   Declarator { add_constructor(($2).token, ($2).argument_type, "", (int8_t) 0, ($2).scope); } ConstructorBody { }
;
...
|   Lcurly { current_scope++; } ExplicitConstructorInvocation BlockStatements Rcurly { clear_current_scope(symmbol_table_pass2[current_class], current_scope); current_scope--; }
|   Lcurly { current_scope++; } ExplicitConstructorInvocation Rcurly { clear_current_scope(symmbol_table_pass2[current_class], current_scope); current_scope--; }
|   Lcurly { current_scope++; } Rcurly { clear_current_scope(symmbol_table_pass2[current_class], current_scope); current_scope--; }
;
...
|   This Lparen ArgumentList Rparen {
                                        ...
                                        if(!check_function_in_class( symmbol_table_pass1[current_class], current_class, ($3).argument_type, "", CONSTRUCTOR)){
                                            cerr << "No such constructor present in class\n";
                                            exit(-1);
                                        }
                                    }
...
|   Lcurly Rcurly { /*empty array*/ ($$).type = ""; }
|   Lcurly VariableInitializerList Comma Rcurly { $$ = $2; }
|   Lcurly Comma Rcurly { ($$).type = ""; }
;
...
VariableInitializerList:
    VariableInitializerList Comma VariableInitializer { 
                                                            ...
                                                            if(!check_return_type(($1).type),($3).type){
                                                                cerr<<"Error: Mismatch type "<<($3).type<<" , "<<($1).type<<endl;
                                                                exit(1);
                                                            }
...
FieldAccess:    Primary Dot Identifier  { 
                                            $$ = find_variable_in_class(symmbol_table_pass1[($1).type],$3);
                                        }
...
ArrayAccess:    TypeName Lsquare Expression Rsquare { 
                                                        ...
                                                        if(($1).token == "" ){
                                                            cerr << "Variable not declared\n";
                                                            exit(-1);
                                                        } 

                                                        if(($1).variable_init_status == __UNINITIALIZED) {
                                                            cerr << "Variable not initialized\n";
                                                            exit(-1);
                                                        }
...
MethodInvocation:   MethodTypeName Lparen Rparen {  }
|                   MethodTypeName Lparen ArgumentList Rparen { }
...
ArgumentList:       Expression  { ($$).argument_type = ($1).type; }
|                   ArgumentList Comma Expression { ($$).argument_type = ($1).argument_type + "," + ($2).type; }
;
...
Text_block : TEXT_BLOCK { ($$).token = $1;}
;

Identifier : IDENTIFIER { ($$).token = $1; }
;
