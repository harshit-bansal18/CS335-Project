...
|                   TypeName Lparen ArgumentList Rparen {   
                                                            if(pass_no == 2 ){
                                                                ...
                                                                emit("add "," esp  " + to_string(local_args_sum + 4 +  ret_type_size) + "\t // space for arguments","","");
                                                                for(int i=0; i < paramcount; i++){
                                                                    int offset = i * 4;
                                                                    emit("mov ", args[i], "[esp -" + to_string(offset) + "]", "");
                                                                }
                                                                ...
                                                                if(names_size > 1){
                                                                    emit("mov", $1->threeac, "\t // Push object reference in stack", "");
                                                                    method_name = threeac_filename(($1)->names[names_size-2]->type->name, mname, method_def_pair.second);
                                                                } else {
                                                                    string temp = get_temp();
                                                                    emit("mov", "[ebp-0x10]", temp, "");
